...
def assumptions_panel():
    return html.Div([
        ...
        dbc.Row([
            ...
            dbc.Col([
                section_label("License Terms"),
                field("Upfront ($M)", dbc.Input(id="in-upfront", value=2.0, type="number", step=0.5, size="sm")),
                field("Dev Milestone ($M)", dbc.Input(id="in-dev-mil", value=1.0, type="number", step=0.5, size="sm")),
                field("Reg Milestone ($M)", dbc.Input(id="in-reg-mil", value=2.0, type="number", step=0.5, size="sm")),
                field("Comm Milestone ($M)", dbc.Input(id="in-comm-mil", value=0.0, type="number", step=0.5, size="sm")),
                html.Div(style={"height": "8px"}),
                section_label("Royalty Tiers"),
                field("Tier 1 Rate (%)", dbc.Input(id="in-roy-tier1", value=5.0, type="number", step=0.5, size="sm")),
                field("Tier 2 Rate (%)", dbc.Input(id="in-roy-tier2", value=7.0, type="number", step=0.5, size="sm")),
                field("Tier 3 Rate (%)", dbc.Input(id="in-roy-tier3", value=9.0, type="number", step=0.5, size="sm")),
                html.Div(style={"height": "8px"}),
                field("Tier 1 Threshold ($M)", dbc.Input(id="in-tier1-thresh", value=100.0, type="number", step=10, size="sm")),
                field("Tier 2 Threshold ($M)", dbc.Input(id="in-tier2-thresh", value=200.0, type="number", step=10, size="sm")),
            ], lg=3, md=6),
            dbc.Col([
